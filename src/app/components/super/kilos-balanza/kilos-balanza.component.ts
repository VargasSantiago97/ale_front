...
@Component({
    selector: 'app-kilos-balanza',
    templateUrl: './kilos-balanza.component.html',
    styleUrls: ['./kilos-balanza.component.css']
})
export class KilosBalanzaComponent {
    ...
    ngOnInit() {
        this.cols = [
            ...
            { field: "kg_diferencia", header: "Bal-Tolv" },
    ...
    movimientoToMostrarTabla(mov: any) {
        ...
        var kg_neto = mov.kg_neto ? parseInt(mov.kg_neto) : 0
        var kg_campo = mov.kg_campo ? parseInt(mov.kg_campo) : 0
        dato.kg_diferencia = kg_neto - kg_campo
        ...
        if (dato.ok_balanza) {
            const movimiento = this.db_locales['movimientos'].find((movi: any) => { return movi.id_movimiento == mov.id })
            if (movimiento) {
                dato.neto = movimiento.kg_balanza
                dato.regulacion = movimiento.kg_regulacion
                dato.final = movimiento.kg_salida
            }
        }
    ...
    //de a uno
    setearKilosEnMovimiento(registro: any) {

        var movLocal: any = this.db_locales['movimientos'].find((m: any) => { return m.id_movimiento == registro.id })
        if(!movLocal){
            alert('No existe mov local')
            return
        }
    ...
    setearKilosEnMovimientos(modificando:any){
        if(modificando < this.selectedTablaInicio.length){

            var movLocal: any = this.db_locales['movimientos'].find((m: any) => { return m.id_movimiento == this.selectedTablaInicio[modificando].id })
            if(!movLocal){
                alert('No existe mov local')
                return
            }

            if (this.selectedTablaInicio[modificando].neto && this.selectedTablaInicio[modificando].final) {
                movLocal.kg_neto = this.selectedTablaInicio[modificando].neto
                movLocal.kg_regulacion = this.selectedTablaInicio[modificando].regulacion
                movLocal.kg_neto_final = this.selectedTablaInicio[modificando].final

                movLocal.ok_balanza = 1

                this.editarDB('movimientos', movLocal, () => {
                    this.selectedTablaInicio[modificando].ok_balanza = true

                    this.setearKilosEnMovimientos(modificando+1)
                })
            }
        }
    }

    setearAcond(registro: any) {
        ...
        movLocal.ok_acondicionadora = 1

        this.editarDB('movimientos', movLocal, () => {
            registro.ok_acondicionadora = true
        })

    }
    setearAconds(modificando:any){
        if(modificando < this.selectedTablaInicio.length){
            ...
            this.editarDB('movimientos', movLocal, () => {
                this.selectedTablaInicio[modificando].ok_acondicionadora = true

                this.setearAconds(modificando+1)
            })
        }
    }
